...
public class ItgViewOffsetBehavior<V extends View> extends Behavior<V> {
    private ItgViewOffsetHelper viewOffsetHelper;
    private int tempTopBottomOffset = 0;
    private int tempLeftRightOffset = 0;

    public ItgViewOffsetBehavior() {
    }

    public ItgViewOffsetBehavior(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    public boolean onLayoutChild(CoordinatorLayout parent, V child, int layoutDirection) {
        ...
        if (this.viewOffsetHelper == null) {
            this.viewOffsetHelper = new ItgViewOffsetHelper(child);
        }
